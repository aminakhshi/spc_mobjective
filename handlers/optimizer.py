...
class ConfigLoader:
    ...
    """
    cfg_path: Optional[str] = None # Path to the config file
...
class NeuronSimulationOptimizer:
    ...
    def _load_params(self, file_path: Path) -> Dict[str, Tuple[float, float]]:
        ...
        try:
            with open(file_path, 'r') as f:
                ranges = json.load(f)
            # Basic validation (check if values are lists/tuples of size 2)
    ...
    def feature_extractor(self,
        ...
        ## TODO: If time is not in ms, remove division by 1000 and adjust feature thresholds for correct feature detection 
    ...
    def get_normalized_loss(self,
        ...
        if feature_cols is not None:
            # Keep intersection columns between data and model
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        if self.fit_mode in ['spike', 'all']:
            ...
            if spike_valid:
                ...
                loss_spike = self.max_loss # Assign max loss if model fails
        ...
        if self.multi_objective and self.fit_mode == "all":
            # Return losses separately if multi-objective
            final_loss = (loss_spike, loss_spiketime)
        else:
            # Combine the losses if single-objective
    ...
    def log_best_trials(self, study: optuna.Study, return_solution=True, ode_run=True):
        ...
        """
        logger.info("Optimization completed. Best trials...")
        ...
        # Handling single or multi-objective cases
        if n_obj == 1:
        ...
        csv_path = self.result_dir / "best_trial.csv"
        ...
        for trial in study.best_trials:
            ...
            ## TODO: Check the time if dt is in sec....
            if isinstance(solution, dict) and "error" in solution:
